...
class VDE_Project {
    ...
    public function __construct($path) {
        ...
        $this->meta = array(
            ...
            'order'       => intval($config['executionorder'])
        );
    ...
    public function getNavigation() {
        ...
        foreach (scandir($dir) as $menuName) {
            if (file_exists($menuItemFile = "$dir/$menuName/$menuName.php")) {
                $menuItem            = include($menuItemFile);
                $menuItem['name']    = $menuName;
                $menuItem['navtype'] = 'tab';
                $menuItem['links']   = array();
                foreach (scandir("$dir/$menuName") as $linkFileName) {
                    if ($linkFileName == "$menuName.php" or substr($linkFileName, -4) != '.php') {
                        continue;
                    }
                    $linkData                     = include("$dir/$menuName/$linkFileName");
                    $linkName                     = substr($linkFileName, strlen($menuName) + 1, -4);
                    $linkData['name']             = $linkName;
                    $linkData['navtype']          = 'link';
                    $linkData['parent']           = $menuName;
                    $menuItem['links'][$linkName] = $linkData;
                }
                $menuItems[] = $menuItem;
            }
        }
